#!/usr/bin/env python3
# server.py (V2.6 - AI's Autonomous Enhanced Formatting)
...
def cleanup_on_exit():
    ...
    print("\n[AI-Core] Server shutting down...")
    print("[AI-Core] Autonomous state will reset on next launch.")
...
def get_log_content_by_signature(log_file_path, state_key):
    ...
    if STATE["is_first_run"]:
        ...
        print(f"🧠 [AI-Core] Baseline set for {log_file_path}. Sending full file ({len(raw_lines)} lines).")
...
def health_check():
    print("\n✅ SUCCESS: AI-Core (V2.6) connected to frontend.\n")
...
def get_log_updates():
    ...
    # --- (NEW FORMATTING LOGIC V2.6 - AI'S FIX) ---
...
{formatted_activity_logs}"""
    ...
    print(f"✅ Sending {len(report_message)} chars of NEW log data to AI.")
    return jsonify({"status": "report_ready", "output": report_message.strip()}) # .strip() to remove leading/trailing whitespace

if __name__ == '__main__':
    print("--- Starting AI-Core Server (V2.6 - Enhanced Formatting) ---")
